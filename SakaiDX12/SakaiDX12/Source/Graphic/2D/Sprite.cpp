...
bool Sprite::StaticInitialize(ID3D12Device* device, int window_width, int window_height)
{
	...
	result = D3DCompileFromFile(
		L"Source/Shader/SpriteVertexShader.hlsl",	// シェーダファイル名
		nullptr,
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Source/Shader/SpritePixelShader.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
bool Sprite::LoadTexture(UINT texnumber, const wchar_t* filename)
{
	...
	//無いと動かない
	auto HEAP_PROPERTIES = CD3DX12_HEAP_PROPERTIES(D3D12_CPU_PAGE_PROPERTY_WRITE_BACK, D3D12_MEMORY_POOL_L0);
	// テクスチャ用バッファの生成
...
bool Sprite::Initialize()
{
	...
	auto RESOURCE_DESC_Buffer = CD3DX12_RESOURCE_DESC::Buffer(sizeof(VertexPosUv) * vertNum);
	// 頂点バッファ生成
	result = device->CreateCommittedResource(
		&HEAP_PROPERTIES,
		D3D12_HEAP_FLAG_NONE,
	...
	RESOURCE_DESC_Buffer = CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferData) + 0xff) & ~0xff);
	// 定数バッファの生成
	result = device->CreateCommittedResource(
